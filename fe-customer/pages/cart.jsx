...
const CartPage = () => {
    ...
    const handlePlaceOrderPayOS = useCallback(async (values, details, data) => {
        if (productList.length) {
            try {
                const orderItems = productList.map((product) => {
                    return {
                ...
                const order = {
                    ...
                    shipping: shippingcost,
                    delivery_charges: deliveryCharges.price,
                    order_id: values.orderCode
                };
                let respone = await orderService.placeOrderPayOs(order);
                if (respone.status === 200) {
                    clearCart();
                    swtoast.success({ text: 'PayOs payment successful' });
                }
                    ...
                    <div className="col-12 col-lg-7 cart-left-section">
                        {
                            ...
                            <CustomerInforForm
                                ...
                                amount={finalTotal}
                                handlePlaceOrderPayOS={handlePlaceOrderPayOS}
                            />
                        }
                    </div>
